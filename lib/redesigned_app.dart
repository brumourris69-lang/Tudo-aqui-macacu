...
class _HomeEditorState extends State<HomeEditor> {
  ...
  bool categories = true, highlights = true, offers = true, events = true, tourism = true, saving = false;
  @override void dispose() { title.dispose(); search.dispose(); highlightsTitle.dispose(); offersTitle.dispose(); super.dispose(); }
  Future<void> load() async { final d = (await FirebaseFirestore.instance.collection('home_pages').doc('draft').get()).data() ?? {}; title.text = (d['heroTitle'] ?? 'O que você procura hoje?').toString(); search.text = (d['searchPlaceholder'] ?? 'Encontre em Macacu...').toString(); highlightsTitle.text = (d['highlightsTitle'] ?? 'Tá bombando em Macacu 🔥').toString(); offersTitle.text = (d['offersTitle'] ?? 'Ofertas em Macacu').toString(); final s = Map<String,dynamic>.from(d['sections'] ?? {}); setState(() { categories=s['categories'] ?? true; highlights=s['highlights'] ?? true; offers=s['offers'] ?? true; events=s['events'] ?? true; tourism=s['tourism'] ?? true; }); }
