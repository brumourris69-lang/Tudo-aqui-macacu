...
class _AdCarouselState extends State<AdCarousel> {
  ...
  @override Widget build(BuildContext context) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: FirebaseFirestore.instance.collection('ads').where('published', isEqualTo: true).snapshots(), builder: (context, snapshot) { final remote = snapshot.data?.docs.map((d) => d.data()).toList() ?? []; final ads = remote.isEmpty ? fallbackAds.map((title) => {'title': title, 'description': '', 'link': ''}).toList() : remote; return SizedBox(height: 174, child: PageView.builder(controller: controller, itemCount: ads.length, itemBuilder: (_, i) { final ad = ads[i]; final title = (ad['title'] ?? '').toString(); final description = (ad['description'] ?? '').toString(); final link = (ad['link'] ?? '').toString(); return Padding(padding: const EdgeInsets.fromLTRB(20, 18, 4, 0), child: Material(color: Colors.transparent, child: InkWell(onTap: link.isEmpty ? null : () => openUrl(context, link, title), borderRadius: BorderRadius.circular(22), child: DecoratedBox(decoration: BoxDecoration(gradient: const LinearGradient(colors: [ocean, sky]), borderRadius: BorderRadius.circular(22)), child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('ESPAÇO PUBLICITÁRIO', style: TextStyle(color: yellow, fontWeight: FontWeight.w800, fontSize: 10)), const Spacer(), Text(title, style: const TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.w800)), const SizedBox(height: 4), Text(description.isEmpty ? 'Toque para saber mais' : description, style: const TextStyle(color: Color(0xFFDDF4FF)))]))))); })); });
