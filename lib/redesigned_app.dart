...
class HomeView extends StatelessWidget {
  ...
  final User? user;
  @override
  Widget build(BuildContext context) => StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(stream: FirebaseFirestore.instance.collection('home_pages').doc('published').snapshots(), builder: (context, snapshot) { final config=snapshot.data?.data() ?? const <String,dynamic>{}; final sections=Map<String,dynamic>.from(config['sections'] ?? const {}); final hero=(config['heroTitle'] ?? 'O que você procura hoje?').toString(); final search=(config['searchPlaceholder'] ?? 'Encontre em Macacu...').toString(); final highlightsTitle=(config['highlightsTitle'] ?? 'Tá bombando em Macacu 🔥').toString(); final offersTitle=(config['offersTitle'] ?? 'Ofertas em Macacu').toString(); return CustomScrollView(slivers: [
        SliverToBoxAdapter(child: WelcomeHero(onSearch: () => showSearch(context: context, delegate: CitySearch()), user: user, title: hero, searchPlaceholder: search)),
        const SliverToBoxAdapter(child: AdCarousel()),
        if(sections['categories'] != false) SliverToBoxAdapter(child: SectionTitle(title: 'Categorias', action: 'Ver todas', onTap: showExplore)),
        ...
        if(sections['highlights'] != false) SliverToBoxAdapter(child: SectionTitle(title: highlightsTitle, action: 'Ver todos', onTap: showExplore)),
        if(sections['highlights'] != false) SliverToBoxAdapter(child: PublishedBusinessStrip(saved: saved, favorite: favorite)),
        if(sections['offers'] != false) SliverToBoxAdapter(child: SectionTitle(title: offersTitle, action: 'Ver ofertas', onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const OffersView())))),
        if(sections['offers'] != false) const SliverToBoxAdapter(child: Padding(padding: EdgeInsets.symmetric(horizontal: 20), child: OfferBanner())),
        if(sections['resources'] != false) SliverToBoxAdapter(child: SectionTitle(title: 'Vantagens e avisos', action: 'Abrir', onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const ResourcesHub())))),
        if(sections['resources'] != false) const SliverToBoxAdapter(child: Padding(padding: EdgeInsets.symmetric(horizontal: 20), child: ResourcesPreview())),
        if(sections['jobs'] != false) SliverToBoxAdapter(child: SectionTitle(title: 'Novos por aqui', action: 'Ver todas', onTap: () => openFeature(context, Feature.jobs))),
        if(sections['jobs'] != false) SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Column(children: jobs.take(2).map((job) => Padding(padding: const EdgeInsets.only(bottom: 10), child: JobCard(job: job))).toList()))),
        if(sections['events'] != false) SliverToBoxAdapter(child: SectionTitle(title: 'O que tá rolando', action: 'Ver notícias', onTap: () => openFeature(context, Feature.news))),
        if(sections['events'] != false) SliverToBoxAdapter(child: const Padding(padding: EdgeInsets.symmetric(horizontal: 20), child: LocalNewsCard())),
        if(sections['events'] != false) SliverToBoxAdapter(child: SectionTitle(title: 'Agenda Macacu', action: 'Ver agenda', onTap: () => openFeature(context, Feature.events))),
        if(sections['events'] != false) SliverToBoxAdapter(child: const Padding(padding: EdgeInsets.symmetric(horizontal: 20), child: EventCard())),
        if(sections['tourism'] != false) SliverToBoxAdapter(child: SectionTitle(title: 'Descubra Macacu', action: 'Explorar agora', onTap: () => openFeature(context, Feature.tourism))),
        if(sections['tourism'] != false) SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 28), child: NatureBanner(onTap: () => openFeature(context, Feature.tourism)))),
      ]); });
}

class WelcomeHero extends StatelessWidget {
  const WelcomeHero({super.key, required this.onSearch, required this.user, required this.title, required this.searchPlaceholder});
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(title, style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.w800, height: 1.03, letterSpacing: -1)),
          const SizedBox(height: 4),
          const Text('Cachoeiras de Macacu • RJ', style: TextStyle(color: muted, fontSize: 12, fontWeight: FontWeight.w600)),
          const SizedBox(height: 18),
          Material(color: Colors.white, borderRadius: BorderRadius.circular(16), child: InkWell(onTap: onSearch, borderRadius: BorderRadius.circular(16), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 16), child: Row(children: [const Icon(Icons.search_rounded, color: ocean), const SizedBox(width: 10), Expanded(child: Text(searchPlaceholder, style: const TextStyle(color: muted))), const Icon(Icons.tune_rounded, color: sky)])))),
        ]),
      );
}
            ...
            return Padding(
              ...
              child: Material(
                ...
                child: InkWell(
                  ...
                  child: DecoratedBox(
                    decoration: BoxDecoration(gradient: imageUrl.isEmpty ? const LinearGradient(colors: [ocean, sky]) : const LinearGradient(colors: [Color(0x99206090), Color(0x99206090)]), image: imageUrl.isEmpty ? null : DecorationImage(image: NetworkImage(imageUrl), fit: BoxFit.cover), borderRadius: BorderRadius.circular(22)),
                    child: Padding(
...
class _HomeEditorState extends State<HomeEditor> {
  ...
  final highlightsTitle = TextEditingController();
  final offersTitle = TextEditingController();
  bool categories = true, highlights = true, offers = true, events = true, tourism = true, resources = true, jobs = true, saving = false;
  @override void dispose() { title.dispose(); search.dispose(); highlightsTitle.dispose(); offersTitle.dispose(); super.dispose(); }
  Future<void> load() async { final db = FirebaseFirestore.instance; final draft = await db.collection('home_pages').doc('draft').get(); final d = draft.data() ?? (await db.collection('home_pages').doc('published').get()).data() ?? {}; title.text = (d['heroTitle'] ?? 'O que você procura hoje?').toString(); search.text = (d['searchPlaceholder'] ?? 'Encontre em Macacu...').toString(); highlightsTitle.text = (d['highlightsTitle'] ?? 'Tá bombando em Macacu 🔥').toString(); offersTitle.text = (d['offersTitle'] ?? 'Ofertas em Macacu').toString(); final s = Map<String,dynamic>.from(d['sections'] ?? {}); setState(() { categories=s['categories'] ?? true; highlights=s['highlights'] ?? true; offers=s['offers'] ?? true; events=s['events'] ?? true; tourism=s['tourism'] ?? true; resources=s['resources'] ?? true; jobs=s['jobs'] ?? true; }); }
